...
object YukiKanaConverter {
    ...
    init {
        ...
        // 促音を追加する
        for ((romaji, hiragana) in builder.build().entries) {
     ...
     */
    private fun canStartFromSokuon(romaji: String): Boolean = !StringUtils.startsWithAny(romaji, "a", "i", "u", "e", "o", "n")

    /**
     * 「ローマ字」から「かな文字」に変換する
     */
    fun conv(romaji: String): String {
        val result = StringUtils.replaceEach(romaji, ROMAJI_LIST, HIRAGANA_LIST)
        return result
    }
}
